...
parser.add_argument(
    "--max_epochs", type=int, default=10, help="number of sequences to train"
)
parser.add_argument(
    "--reset_hidden", action="store_true", help="reset the hidden state of the RNN"
)
parser.add_argument(
    "--model_type", type=str, default="rnn", help="model to use for training (rnn|s4)"
)

parser.add_argument(
    "--experiment_name",
    type=str,
    default="repeat_copy_test",
...
parser.add_argument(
    "--seq_length", type=int, default=7, help="sequence length for the memory"
)
...
if "EXPERIMENT_OUTPUT_DIR" not in os.environ:
    print(
        "Please set the environment variable EXPERIMENT_OUTPUT_DIR to the directory where the logs should be saved"
    )
    sys.exit()
...
if args.model_type == "rnn":
    ...
    model = S4Model(
        input_dim, hidden_dim, output_dim, l_max=args.seq_length * 10, bias=False
    )
else:
    print("Model type not recognized")
    sys.exit()

lightning_module = RepeatCopyTask(
    model,
    seed=seed,
