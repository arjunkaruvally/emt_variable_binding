...
class PolyEpisodicRNNModelwDelay(torch.nn.Module):
    ...
    def execute_cell(self, input):
        ...
        for pos_id in range(seq_length):
            # in_arr =
            z = torch.nn.functional.sigmoid(
                (input[pos_id, :, :].squeeze()) @ self.w_ic
            ...
            x = (
                torch.tanh(self.h) @ self.w_hh0.T
                + (input[pos_id, :, :].squeeze()) @ self.w_ih
                + self.c @ self.w_ch.T
                + self.b1.T
            )
    ...
    def initialize_hidden(self, batch_size=1, h=None, c=None, device="cpu"):
        if h is None:
            self.h = (
                torch.zeros(
                    batch_size, self.hidden_dim, device=device, requires_grad=True
                )
                + 1e-3
            )
            self.c = (
