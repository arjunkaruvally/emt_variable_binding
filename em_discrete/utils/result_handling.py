...
def parse_directory(path, hparam_names=None):
    """
    ...
    SafeLoaderIgnoreUnknown.add_constructor(
        "tag:yaml.org,2002:python/tuple", SafeLoaderIgnoreUnknown.tuple
    )
    SafeLoaderIgnoreUnknown.add_constructor(
            ...
            with open(hparam_file, "r") as f:
                ...
                try:
                    checkpoint_files = os.listdir(
                        os.path.join(path, filename, "checkpoints")
                    )
                ...
                if len(checkpoint_files) > 0:
                    ...
                    result_dict["path"].append(
                        os.path.join(
                            path, filename, "checkpoints", checkpoint_files[-1]
                        )
                    )
