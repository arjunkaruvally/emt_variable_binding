...
class CopyingMemoryDataset(IterableDataset):
    ...
    def __iter__(self):
        x_sample = torch.zeros(
            (self.final_seq_length, self.batch_size, self.d + 2)
        )  # +2 is for the extra signals
        y_sample = torch.zeros(
            (self.final_seq_length, self.batch_size, self.d + 2)
        )  # +2 is for the extra signals
        ...
        for batch_id in range(self.batch_size):
            category_list = torch.LongTensor(
                np.random.choice(range(self.d), self.seq_length)
            )

            x_sample[: self.seq_length, batch_id, :] = F.one_hot(
                category_list, num_classes=self.d + 2
            )
            y_sample[self.T + self.seq_length :, batch_id, :] = F.one_hot(
                category_list, num_classes=self.d + 2
            )
...
class CopyingMemoryTask(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        full_train_dataset = CopyingMemoryDataset(
            seed=self.seed,
            d=self.input_dim,
        ...
        full_val_dataset = CopyingMemoryDataset(
            seed=self.seed,
            d=self.input_dim,
            batch_size=batch_size,
            seq_length=self.seq_length,
        ...
        full_test_dataset = CopyingMemoryDataset(
            seed=self.seed,
            d=self.input_dim,
            batch_size=batch_size,
            seq_length=self.seq_length,
    ...
    def validation_step(self, batch, batch_nb):
        with torch.enable_grad():
            ...
            for i in range(0, self.T + 20):
                ...
                lookbacks.append(i)
                norms.append(
                    torch.linalg.vector_norm(xgrad[i, :, :].squeeze(), dim=1)
                    .mean()
                    .item()
                )
            self.optimizer.zero_grad()

        self.logger.experiment.add_histogram_raw(
            "x_grad norms",
            np.min(norms),
            np.max(norms),
            len(norms),
    ...
    def training_step(self, batch, batch_nb):
        ...
        accuracy = (
            (
                y_hat.argmax(1)[-self.seq_length * self.batch_size :]
                == y.argmax(1)[-self.seq_length * self.batch_size :]
            )
            .float()
            .mean()
        )
