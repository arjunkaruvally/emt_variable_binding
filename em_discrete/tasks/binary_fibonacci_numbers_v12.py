    ...
    def __iter__(self):
        for i in range(5000):
            ...
            a = torch.randint(
                ...
                size=(
                    self.batch_size,
                    int(self.horizon / self.STREAM.ATOMIC_SIZE.value),
                ),
            )
            b = torch.randint(
                0,
            ...
            y_sample[:, :, :] = 0
            y_sample[
                self.STREAM.EVALUATION_FROM.value :,
                :,
                self.STREAM.DATASTREAM_START.value :,
            ] = binarized_stream
            ...
            x_sample[0, :, self.STREAM.DATASTREAM_START.value :] = (
                self.convert_to_binary(a[:, 0:1]) * 2 - 1
            )
            # x_sample[1, :, self.STREAM.EOB.value] = 1
            x_sample[1, :, self.STREAM.DATASTREAM_START.value :] = (
                self.convert_to_binary(b[:, 0:1]) * 2 - 1
            )
...
class BinaryFibonacciNumbersTask(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        full_train_dataset = BinaryFibonacciNumbersDataset(
            seed=self.seed, d=self.input_dim, batch_size=batch_size
        )
        full_val_dataset = BinaryFibonacciNumbersDataset(
            seed=self.seed, d=self.input_dim, batch_size=batch_size
        )
        full_test_dataset = BinaryFibonacciNumbersDataset(
            seed=self.seed, d=self.input_dim, batch_size=batch_size
        )
