...
def composition_func(x, time_idx):
    ...
    kernel = torch.tensor([[1, 1, 1], [1, 0, 1], [1, 1, 1]]).float()
    ...
    output[
        torch.logical_and(
            x[0, :, :].squeeze() == 1, (neighbour_matrix[:, 0, 0, :].squeeze() < 2)
        )
    ] = 0
    ...
    output[
        torch.logical_and(
            x[0, :, :].squeeze() == 1, (neighbour_matrix[:, 0, 0, :].squeeze() > 3)
        )
    ] = 0
    ...
    output[
        torch.logical_and(
            x[0, :, :].squeeze() == 0, (neighbour_matrix[:, 0, 0, :].squeeze() == 3)
        )
    ] = 1
