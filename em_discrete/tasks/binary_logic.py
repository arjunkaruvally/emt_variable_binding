...
class BinaryLogicDataset(IterableDataset):
    ...
    def __iter__(self):
        for i in range(5000):
            ...
            x_sample[
                self.seq_length : 2 * self.seq_length,
                :,
                self.STREAM.IN2_INDICATOR.value,
            ] = 1
            ...
            x_sample[: self.seq_length, :, self.STREAM.DATALINE.value] = (
                self.convert_to_binary(a).T
            )
            x_sample[
                self.seq_length : 2 * self.seq_length, :, self.STREAM.DATALINE.value
            ] = self.convert_to_binary(b).T

            # add operator signal
            x_sample[2 * self.seq_length, :, self.STREAM.LAND.value] = (
                and_mask.flatten()
            )
            x_sample[2 * self.seq_length, :, self.STREAM.LOR.value] = (
                1 - and_mask.flatten()
            )
            ...
            y_sample[input_offset : input_offset + 2 * self.seq_length + 1] = x_sample[
                : 2 * self.seq_length + 1
            ]

            # add logical result to output
            input_offset = 4 * self.seq_length + 3
            y_sample[input_offset:-1, :, self.STREAM.DATALINE.value] = (
                self.convert_to_binary(and_mask * c_and + (1 - and_mask) * c_or).T
            )
...
class BinaryLogicTask(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        full_train_dataset = BinaryLogicDataset(
            seed=self.seed,
            d=self.input_dim,
            batch_size=batch_size,
            seq_length=self.seq_length,
        )
        full_val_dataset = BinaryLogicDataset(
            seed=self.seed,
            d=self.input_dim,
            batch_size=batch_size,
            seq_length=self.seq_length,
        )
        full_test_dataset = BinaryLogicDataset(
            seed=self.seed,
            d=self.input_dim,
            batch_size=batch_size,
            seq_length=self.seq_length,
    ...
    def training_step(self, batch, batch_nb):
        ...
        accuracy = (
            (
                (y_hat[:, InputStream.DATALINE.value] > 0.5).long().flatten()
                == y[:, InputStream.DATALINE.value]
            )
            .float()
            .mean()
        )
