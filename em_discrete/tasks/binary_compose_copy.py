...
class BinaryComposeCopyDataset(IterableDataset):
    ...
    def __init__(self, seed=0, d=10, seq_length=5, batch_size=64, horizon=36, generator_stop=2000):
        self.d = d
    ...
    def __iter__(self):
        for i in range(self.generator_stop):
            ...
            for step in range(self.horizon):
                ...
                compose_seq_id_3 = step + 2

                three_block = self.data_d // 3
                # y_sample[composed_seq_id, :, :three_block] = -y_sample[compose_seq_id_1, :, three_block:2*three_block]
                # y_sample[composed_seq_id, :, three_block:2*three_block] = y_sample[compose_seq_id_2, :, -three_block:]
                # y_sample[composed_seq_id, :, 2*three_block:3*three_block] = y_sample[compose_seq_id_3, :, :three_block]
                # y_sample[composed_seq_id, :, ::2] = - y_sample[composed_seq_id, :, ::2]

                y_sample[composed_seq_id, :, :] = torch.diagonal(y_sample[step:composed_seq_id], dim1=0, dim2=2)
                # y_sample[composed_seq_id, :, three_block:2*three_block] *= -1
...
class BinaryComposeCopyTask(pl.LightningModule):
    def __init__(self, model, seed=0, learning_rate=1e-5, batch_size=32, input_dim=6, seq_length=5,
                 ...
                 curriculum_threshold=0.96, composition_operation="add", **kwargs):
        ...
        full_train_dataset = BinaryComposeCopyDataset(seed=self.seed,
                                                      d=self.input_dim,
                                                      seq_length=self.seq_length,
                                                      batch_size=batch_size)
        full_val_dataset = BinaryComposeCopyDataset(seed=self.seed,
                                                    seq_length=self.seq_length,
                                                    d=self.input_dim,
                                                    batch_size=batch_size)
        full_test_dataset = BinaryComposeCopyDataset(seed=self.seed,
                                                     d=self.input_dim,
                                                     seq_length=self.seq_length,
                                                     batch_size=batch_size)
